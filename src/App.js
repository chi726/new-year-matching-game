  ...
  const ResultEnvelope = ({ pData, showName = true }) => {
    ...
    return (
      <div className="flex flex-col items-center w-full">
        <div onClick={() => toggleEnvelope(displayId)} className="relative h-40 w-full max-w-[140px] md:max-w-[160px] cursor-pointer" style={{ perspective: '1000px' }}>
          <div className={`absolute inset-x-0 transition-all duration-700 flex flex-col items-center z-10 ${isRevealed ? '-translate-y-28 opacity-100 scale-110' : 'translate-y-0 opacity-0'}`}>
            <div className="flex flex-wrap justify-center gap-1 mb-2 max-w-[140px]">
              {cashItems.map((item, i) => (
                <div key={i} className={`${item.color} ${item.type === 'bill' ? 'w-10 h-6' : 'w-6 h-6 rounded-full'} border flex items-center justify-center text-[8px] text-white font-black shadow-md animate-bounce`}>
                  ${item.val}
                </div>
              ))}
  ...
  return (
    <div className="min-h-screen bg-orange-50 text-slate-800 pb-32 overflow-x-hidden font-sans">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-red-100 flex justify-around p-4 z-50 shadow-xl">
        <button onClick={() => setView('landing')} className={`flex flex-col items-center gap-1 ${view === 'landing' || view === 'picking' ? 'text-red-600 font-bold' : 'text-slate-400'}`}><Gift size={24} /><span className="text-[10px]">抽取紅包</span></button>
        <button onClick={() => setView('results')} className={`flex flex-col items-center gap-1 ${view === 'results' ? 'text-red-600 font-bold' : 'text-slate-400'}`}><Trophy size={24} /><span className="text-[10px]">配對結果</span></button>
        <button onClick={() => setView('admin')} className={`flex flex-col items-center gap-1 ${view === 'admin' ? 'text-red-600 font-bold' : 'text-slate-400'}`}><Settings size={24} /><span className="text-[10px]">系統管理</span></button>
      </nav>
      ...
      <main className="max-w-5xl mx-auto mt-6 px-4">
        ...
        {/* 抽取頁 */}
        {view === 'picking' && (
          <div className="max-w-3xl mx-auto mt-4 animate-in slide-in-from-bottom-8">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
              {Array.from({ length: gameConfig.totalEnvelopes || 24 }).map((_, i) => {
                ...
                return (
                  <button key={i} disabled={!!p} onClick={() => handlePick(i)} className={`relative h-24 rounded-xl border-2 transition-all flex flex-col items-center justify-center shadow-md active:scale-95 ${p?.uid === user?.uid ? 'bg-yellow-400 border-yellow-600 scale-105 z-10 shadow-lg' : p ? 'bg-gray-200 border-gray-300 opacity-40 grayscale' : 'bg-red-600 border-yellow-500 hover:scale-105'}`}>
                    ...
                    {!p && <div className="mt-0.5 w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-[7px] text-red-800 border border-yellow-200 font-serif font-bold font-bold font-bold font-bold">福</div>}
                  </button>
                );
              })}
            </div>
            ...
            {!isAdminAuthenticated ? (
              <div className="text-center py-8">
                ...
                <form onSubmit={(e) => { e.preventDefault(); if (adminPasswordInput === ADMIN_PASSWORD) setIsAdminAuthenticated(true); else setError('管理密碼錯誤'); }} className="space-y-6">
                  ...
                  <button className="w-full bg-red-600 text-white font-black py-4 rounded-xl shadow-lg text-lg">驗證身份</button>
                </form>
              </div>
            ) : (
              <div className="space-y-6 animate-in fade-in">
                <div className="flex justify-between items-center border-b-2 border-red-50 pb-2">
                  <h2 className="font-black text-red-900 uppercase flex items-center gap-2"><Settings size={18}/> 管理中心</h2>
                  <button onClick={() => setIsAdminAuthenticated(false)} className="text-xs text-red-400 underline">退出管理</button>
                </div>
                <div className="bg-orange-50/50 p-4 rounded-2xl border border-orange-100 space-y-4">
                  ...
                  <button onClick={generateEnvelopePool} className="w-full bg-orange-600 text-white font-black py-3 rounded-xl shadow-md text-xs flex items-center justify-center gap-2"><Database size={14}/> 重新生成金額池</button>
                </div>
                <div className="bg-red-50/50 p-4 rounded-2xl border border-red-100 space-y-3">
                  <button onClick={handleMatchAndShow} className="w-full bg-red-600 text-white font-black py-4 rounded-xl shadow-xl flex items-center justify-center gap-2"><Trophy size={20}/> 正式配對並公佈結果</button>
                  <div className="flex gap-2">
                    <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'config'), { showAllResults: !gameConfig.showAllResults })} className={`flex-1 py-2 rounded-xl font-bold border-2 transition-all ${gameConfig.showAllResults ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-white text-slate-400'}`}>{gameConfig.showAllResults ? '隱藏結果' : '顯示結果'}</button>
                ...
                return (
                  <div className="max-w-md mx-auto flex flex-col items-center bg-white pt-36 pb-12 px-8 rounded-[3rem] shadow-2xl border-4 border-yellow-500/40 relative animate-in zoom-in">
                    ...
                    <div className="mt-12 text-center bg-red-50 px-8 py-6 rounded-3xl border-2 border-red-100 w-full shadow-inner relative z-10">
                      ...
                      <p className="font-black text-red-800 text-2xl tracking-widest leading-relaxed">
                        {myResult ? (myResult.isPair ? `${myResult.p1.name} ❤️ ${myResult.p2.name}` : `${myResult.p1.name} (大吉獨贏)`) : "等待管理者揭曉..."}
                      </p>
                    </div>
                  </div>
                );
              ...
              {gameConfig.showAllResults ? (
                <div className="animate-in fade-in slide-in-from-bottom-12">
                  <div className="flex items-center gap-6 mb-16 px-4">
                    ...
                    <span className="text-red-500 text-sm font-black tracking-[0.5em] uppercase">全體揭曉名單</span>
                    <div className="h-px bg-red-200 flex-1"></div>
                  </div>
                      ...
                      <div key={idx} className="bg-white/80 backdrop-blur-sm rounded-[3rem] pt-36 pb-14 px-8 border-2 border-red-100 shadow-xl relative group">
                        ...
                        {pair.isPair ? (
                          ...
                          <div className="flex flex-col items-center gap-12 relative z-10">
                            <ResultEnvelope pData={pair.p1} />
                            <div className="text-center font-black text-amber-800 bg-amber-50 px-12 py-4 rounded-full border-2 border-amber-200 shadow-md text-xl tracking-widest">🌟 {pair.p1.name} 大吉大利</div>
                          </div>
                        )}
                      </div>
                    ))}
                ...
                <div className="max-w-md mx-auto text-center p-20 bg-white/40 rounded-[3rem] border-4 border-dotted border-red-200 shadow-inner">
                  <EyeOff size={64} className="mx-auto text-red-200 mb-6" />
                  <p className="text-red-300 font-black text-xl tracking-[0.3em] uppercase">全體名單封存中</p>
                </div>
              )}
            </div>
          </div>
